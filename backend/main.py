...
def create_room(room_data: RoomCreate):
    ...
    for node in nodes:
        edges.append({"source": node, "target": node, "weight": 0.0})
    ...
    for p_node in player_nodes:
        for o_node in other_nodes:
            edges.append({"source": p_node, "target": o_node, "weight": 2.0})
            edges.append({"source": o_node, "target": p_node, "weight": 2.0}) # Add reverse edge
...
async def submit_move(room_id: str, move: Move):
    ...
    if current_points_spent + points_spent > room.points_per_round_K:
        raise HTTPException(status_code=400, detail=f"Exceeded their points limit for this round.")
    ...
    # Check if all players have spent their points
    if room.players and all(room.submitted_moves_points.get(p.id, 0) >= room.points_per_round_K for p in room.players):
